...
from .utils.ws_manager_utils import (
    _get_job_by_id, _get_job_tasks, _get_pending_tasks, _create_job_in_database, 
    _create_tasks_for_job, _create_worker_in_database, _increment_job_completed_tasks, 
    _record_worker_failure, _update_job_status, _update_task_status, _update_worker_status, _update_worker_task_stats
) 
from common.protocol import (
    Message, MessageType, create_assign_task_message,
    create_job_results_message, create_ping_message, create_job_accepted_message
)
...
class WebSocketManager:
    ...
    async def _handle_task_result(self, message: Message, websocket: WebSocketServerProtocol):
        ...
        await _increment_job_completed_tasks(job_id)
    ...
    async def _handle_task_error(self, message: Message, websocket: WebSocketServerProtocol):
        ...
        # await _increment_job_completed_tasks(job_id) #wrong, do not increment if failed
        ...
        if (len(completed_tasks) == len(tasks)) and (len(tasks) > 0):
            ...
            if job:
                ...
                if job_id in self.job_websockets:
                    ...
                    await _update_job_status(job_id, "completed", completed_tasks=len(completed_tasks))
