...
class JobManager:
    ...
    async def mark_task_completed(
        ...
        Returns:
            True if job is now complete, False otherwise
        """
        ...
        if job_id in self._job_metadata:
            self._job_metadata[job_id]['completed_tasks'] += 1
        
        # Check if job is complete
        is_complete = await self.is_job_complete(job_id)
